...
public class PooledByteBufAllocator extends AbstractByteBufAllocator implements ByteBufAllocatorMetricProvider {
    ...
    static {
        ...
        // 确定 heap 堆内存 和 direct 堆外内存的 arena 的合理默认值
        // Determine reasonable default for nHeapArena and nDirectArena.
        // 假设每个 arena 有3个区块，则内存池不应超过最大内存的50％
        // Assuming each arena has 3 chunks, the pool should not consume more than 50% of max memory.
        final Runtime runtime = Runtime.getRuntime();

        // jemalloc 使用 4 倍可用 cpu 核心
        // 这里我们使用 2 倍可用 cpu 核心数的 arena，因为 我们使用另外 2 倍可用 cpu 核心数来处理 NIO 和 EPOLL 重的 EventLoop
        // 这样做可以减少 cpu 争用
        /*
        ...
        // 一个 arena 包含 3 个 chunk，上文提到内存池 大小应该小于 jvm 可用内存的 50%
        // 因此 arena 的数量应该为 maxMemorry / 2 / defaultChunkSize / 3
        // 同时 arena 的数量最合理应该是 可用cpu核心数的 2 倍
        // 取 两个的最小值
        // 堆外内存也是一样的道理
        ...
        // cache sizes
        // 定义缓存大小 分为三个级别 tiny 512b   small 256b   normal 64b
        ...
        // 如同 jemalloc 描述的一样， 我们将 32k 作为最大可缓存数
    ...
    private static int validateAndCalculatePageShifts(int pageSize) {
        ...
        // numberOfLeadingZeros 计算一个数的 32位表示发情况下 左起有多少连续的 0
        // pageShift：pageSize 的偏移量 也即 pageSize 是二的多少倍，pageSize 右边有几个连续的 0
